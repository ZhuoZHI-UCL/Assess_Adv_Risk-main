...
for test_ins in ins_list:
    ...
    for run_iter in range(n_run):
        ...
        for i_iter in range(n_iter):
            ...
            # find the minimum epsilon whose p value is larger than 0.05

            p_value_excess = Y[:,0]<(-0.05)
            #print(p_value_excess)
            #print(np.min(X[p_value_excess,0]))
            epsilon_search_bound = np.min(X[p_value_excess,0])
            print('epsilon_search_bound')
            print(epsilon_search_bound)
            #exit()
            ...
            for t_step in range(n_steps):
                ...
                x_np[:,0]=x_np[:,0]*epsilon_search_bound
            ...
            # shrink the search space
            X_candidate_np[:,0]=X_candidate_np[:,0]*epsilon_search_bound
            ...
            solution['X']=X_sol
            solution['Y']=Y_sol
            solution['coef_lcb']=coef_lcb
            solution['coef_guide']=coef_guide
            with open('result/'+args.save_path, 'wb') as output_file:
        ...
        #with open('result/hv_psl_mobo_0612_guided_search_v1.pickle', 'wb') as output_file:
