...
import pickle
import json
from attack_imagenet_square_attack import get_problem
import time
import argparse
from lhs import lhs
...
x_init= np.concatenate((np.transpose(np.array([[0.432, 0.434, 0.436, 0.438, 0.44]])),np.random.rand(5,2)),axis=1)
print(x_init)
y_init = problem.evaluate(torch.from_numpy(x_init).to(device))
print(x_init)
print(y_init)
pickle.dump(x_init,open('B-Box-eval-square-attack/hyp_search_eps.x.pkl','wb'))
pickle.dump(y_init,open('B-Box-eval-square-attack/hyp_search_eps.y.pkl','wb'))
